"use client";

import { useActionState, useRef } from "react";
import {
  uploadManuscriptAction,
  type UploadManuscriptState,
} from "@/lib/actions/manuscript";

const initialState: UploadManuscriptState = {};

export function ManuscriptUploadForm({
  bookProjectId,
}: {
  bookProjectId: string;
}) {
  const formRef = useRef<HTMLFormElement>(null);
  const [state, formAction, pending] = useActionState(async (
    prevState: UploadManuscriptState,
    formData: FormData,
  ) => {
    const result = await uploadManuscriptAction(prevState, formData);
    if (!result.error) {
      formRef.current?.reset();
    }
    return result;
  }, initialState);

  return (
    <form ref={formRef} action={formAction} className="space-y-3">
      <input type="hidden" name="bookProjectId" value={bookProjectId} />

      <div>
        <label htmlFor="manuscript-file" className="block text-sm font-medium text-slate-700">
          Berkas Naskah (.docx atau .md)
        </label>
        <input
          id="manuscript-file"
          name="file"
          type="file"
          accept=".docx,.md,.markdown"
          required
          className="mt-1 block w-full text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-3 file:py-2 file:text-sm file:font-medium file:text-indigo-700 hover:file:bg-indigo-100"
        />
        <p className="mt-1 text-xs text-slate-500">
          Gambar di dalam naskah belum didukung — versi PDF final akan tanpa
          gambar. Gunakan gaya heading (Word) atau tanda <code>#</code>
          (Markdown) untuk judul bab.
        </p>
      </div>

      {state.error && (
        <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">
          {state.error}
        </p>
      )}

      <button
        type="submit"
        disabled={pending}
        className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-60"
      >
        {pending ? "Mengunggah..." : "Unggah Naskah"}
      </button>
    </form>
  );
}
